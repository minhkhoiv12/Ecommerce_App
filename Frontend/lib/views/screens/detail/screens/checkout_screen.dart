...
class _CheckoutScreenState extends ConsumerState<CheckoutScreen> {
  ...
  Future<void> handleStripePayment(BuildContext context) async {
    ...
    if (user == null) {
      showSnackBar(context, 'Vui lòng đăng nhập để tiếp tục');
      return;
    }
    ...
    try {
      ...
      if (totalAmount <= 0) {
        showSnackBar(context, 'Số tiền không hợp lệ');
        return;
      }
      ...
      await Stripe.instance.initPaymentSheet(
        paymentSheetParameters: SetupPaymentSheetParameters(
          ...
          merchantDisplayName: 'Khoi Store',
        ),
      );
      ...
      showSnackBar(context, 'Lỗi khi xử lý thanh toán: $e');
    } finally {
      setState(() {
        isLoading = false;
      });
